...
namespace Komorebi.Utilities {
	...
	public Pixbuf getIconFrom (string icon, int size) {
		...
		_IconTheme.prepend_search_path("/usr/local/share/pixmaps/");
	...
	void fixConflicts() {
		...
		SettingsSchemaSource settingsSchemaSource = new SettingsSchemaSource.from_directory ("/usr/local/share/glib-2.0/schemas", null, false);
	...
	void readWallpaperFile () {
		...
		var wallpaperPath = @"/usr/local/share/Komorebi/Resources/Komorebi/$wallpaperName";
		...
		if(wallpaperName == null || !File.new_for_path(wallpaperPath).query_exists() ||
			...
			wallpaperName = "foggy_sunny_mountain";
			wallpaperPath = @"/usr/local/share/Komorebi/Resources/Komorebi/$wallpaperName";
		...
		var wallpaperJpgPath = @"/usr/local/share/Komorebi/Resources/Komorebi/$wallpaperName/wallpaper.jpg";
	...
	public bool canPlayVideos() {
		...
		if(	File.new_for_path("/usr/local/lib/gstreamer-1.0/libgstlibav.so").query_exists() ||
